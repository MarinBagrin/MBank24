...
class HomeVC:UIViewController{
    ...
    init(coordinator:HomeCoordinator, mainDomain:MainDomain, appVM:AppVM) {
        self.coordinator = coordinator
        self.viewModel = HomeVM(mainDomain: mainDomain)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            if (row == 0) {
                ...
                cell = cell0
                cell.textLabel?.text = "Счет"
                cell.textLabel?.textColor = UIColor(red: 0, green: 0.1, blue: 1, alpha: 1)
                cell.textLabel?.font = .boldSystemFont(ofSize: 18)
                cell.detailTextLabel?.text = String(viewModel.getAccount)
